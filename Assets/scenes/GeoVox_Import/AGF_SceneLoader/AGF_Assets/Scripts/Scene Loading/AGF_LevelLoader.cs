...
//	private bool useEncryption = false;
	...
	List<string> alreadyLoadedBundles = new List<string>();
...
//		alreadyLoadedBundles = new List<string> ();
	}
...
//					print ("bundle " + m_RequiredBundlesForLoadModel[i] + " not loaded.");	
	...
	int bundlesLoading = 0;
	List<string> loadedBundles;
	List<string> missingBundles = new List<string>();

	void GeovoxUpdate(){
		
		if (bundleString != "") {
			string[] split = bundleString.Split( new char[]{'@'} );
			...
			foreach(string path in split){
				...
				foreach(string bundle in loadedBundles){
					if(bundle == Path.GetFileName(path))
						bundleAlreadyLoaded = true;
				}

				if(!bundleAlreadyLoaded){
					string newDirectory = Directory.GetParent(Directory.GetParent(scenePath).ToString()) + "/Asset Packs/" + Path.GetFileName(path);
					if(File.Exists(path.Replace("file:///", ""))){
						loadedBundles.Add(Path.GetFileName(path));
						LoadAssetBundle(path);
						bundlesLoading++;
					}
					else if(File.Exists(newDirectory.Replace("file:///", ""))){
						loadedBundles.Add(Path.GetFileName(path));
						LoadAssetBundle(newDirectory);
						bundlesLoading++;
					}
					else if(File.Exists(Main.GetHomeFolder() + "/Asset Packs/" + Path.GetFileName(path))){
						loadedBundles.Add(Path.GetFileName(path));
						LoadAssetBundle(Main.GetHomeFolder() + "/Asset Packs/" + Path.GetFileName(path));
						bundlesLoading++;
					}
					else{
						missingBundles.Add(Path.GetFileName(path));
					}
				}

			}

			// Tell of missing bundles
			foreach(string missingBundle in missingBundles){
				bool bundleAlreadyLoaded = false;
				foreach(string bundle in loadedBundles){
				...
				if(!bundleAlreadyLoaded)
					Debug.LogError("Could not find bundle at: " + missingBundle);
			}
		...
		Geovox.VoxelInterface voxelInterface = FindObjectOfType<Geovox.VoxelInterface> ();
		Geovox.VoxelTerrain voxelTerrain = FindObjectOfType<Geovox.VoxelTerrain>();
		if(!voxelTerrain){
			GameObject newTerrain = GameObject.Instantiate(voxelInterface.geovoxPrefab) as GameObject;
			voxelTerrain = newTerrain.AddComponent<Geovox.VoxelTerrain>();
			newTerrain.SetActive(true);

			voxelTerrain.data = voxelInterface.data;
			voxelTerrain.types = voxelInterface.geovoxPrefab.GetComponent<Geovox.VoxelTerrain>().types;
			voxelTerrain.Display(true);
		}

		if (settingsString != "") {
			voxelInterface.SetSettings(settingsString);
		...
		if (voxelString != "") {
			voxelTerrain.data.LoadFromString( voxelString );
		...
		FindObjectOfType<EventHandler>().dispatchEvent( new AGFEventObj( AGFEventObj.SceneLoaded ) );
		m_SceneLoaded = true;
	...
	public void EditorUpdate(){
		...
		if ( m_AssetBundleQueue.Count > 0 ){
			Debug.Log(m_AssetBundleQueue.Count + " Bundles Left");

			if(File.Exists(m_AssetBundleQueue.Peek().Key.url.Substring(8, m_AssetBundleQueue.Peek().Key.url.Length - 8)) 
			   && !alreadyLoadedBundles.Contains(m_AssetBundleQueue.Peek().Key.url)
			   ){
				alreadyLoadedBundles.Add(m_AssetBundleQueue.Peek().Key.url);
...
//		Transform[] objList = FindObjectsOfType<Transform> ();
//		for(int i = 0; i < objList.Length; i++){
//			if(objList[i] && objList[i].root.name != "Geovox_Integration")
//				DestroyImmediate(objList[i].root);
//		}
...
//			return AGF_ReturnCode.AssetBundleDataMismatch;	
	...
	private int FinishLoadModel(){
		...
		// Inform of load success.
		FindObjectOfType<EventHandler>().dispatchEvent( new AGFEventObj( AGFEventObj.ModelLoaded ) );
	...
	public void UnloadBundle( string bundleName, bool removeFromList = true ){
		...
		if ( currentActiveBundles.ContainsKey( bundleName ) ){
			currentActiveBundles[bundleName].Unload(true);
		}
...
//		bundle.Unload(false); // all objects have been copied out of the bundle, so unload the bundle itself.
		...
		FindObjectOfType<EventHandler>().dispatchEvent( new AGFEventObj( AGFEventObj.AssetBundleLoaded ) );
...
//							AssetBundleCreateRequest acr = AssetBundle.LoadFromMemoryAsync( decryptedBytes );
//							yield return acr;
//							
...
//					}
		...
		bundlesLoading--;
		print ("Bundles in list: " + bundlesLoading);
	}
